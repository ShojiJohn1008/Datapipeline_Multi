...
安定した新規または更新済み `.m4a` / `.qta` はSHA-256で `JobStore` に `media_type='audio/mp4'` として登録される。新しいmacOS Voice Memosの`.qta`もAACを含むQuickTimeコンテナとして原形式のまま扱う。audio workerはこのMIME種別だけをclaimするので、PDF pending jobには触れない。原本は `Archive/YYYY/MM/voice-memo--<hash8>.<source-ext>`、文字起こしは同じbasenameの`.transcript.json` sidecar、カードは `Cards/YYYY/MM/voice-memo--<hash8>.md` にatomicに公開する。sidecarはschema/version、source/content hash、UTF-8全文の`transcript_sha256`、transcriber/model/language、generated_at、全文transcriptを含む。3つすべてがdurableな後だけcompletedにする。カードにはArchive相対の`transcript_path`と本文内sidecar参照を残す。文字起こしの失敗時は安全な短いエラーcodeだけをfailed jobへ記録し、既存sidecarが正しいhashかつ本文checksumも一致すれば再文字起こしせず再試行する。任意の生成AI要約が失敗しても決定論的カードへフォールバックしてcompletedにする。transcriptや録音内容はSQLiteへ保存しない。

Voice Memos固有の `KH_VOICE_MEMOS_STATE` JSON はJobStoreとは別である。既定は共有SQLiteの隣、`~/Library/Application Support/Datapipeline_Multi/voice_memos_observations.json` である。これは初回baseline、同期遅延cutover、同一statを二回観測する安定性だけを保持し、content hash、attempt、failed/completed、出力パスを保存しない。iCloudが古い録音へ新しいfilesystem mtimeを付けてダウンロードする場合があるため、Apple内部ファイル名が標準の日時形式なら録音日時をcutover判定に使い、未知の名前だけmtimeへfallbackする。初回には `--baseline-existing`（推奨）または明示的な `--backfill-existing` が必要である。baseline済みstateへのbackfill指定は拒否するため、意図的な過去分再投入では先にJSONを退避する。

文字起こしコマンドは `KH_AUDIO_TRANSCRIBE_CMD`（または `--transcribe-cmd`）をshlexでargv化し、Archiveコピーのパスを最後の引数として渡す。stdoutだけをtranscriptとして使い、既定7200秒のtimeoutは `KH_AUDIO_TRANSCRIBE_TIMEOUT` または `--transcribe-timeout` で調整できる。semantic summaryには明示設定の `KH_AUDIO_SUMMARY_CMD`（または `--summary-cmd`）だけを共有 `CliAgentProvider` として使う。未設定では外部CLIを暗黙起動せず、決定論的fallback cardを作る。設定した要約コマンドが失敗しても同じfallbackを作る。カードの`card_generation`は`deterministic`、`generative_ai`、`deterministic_ai_fallback`のいずれかで生成経路を示す。共有providerのJSON契約を用いるので、要約コマンドやそのprompt運用は音声コードに固定しない。`--max-attempts` は文字起こしなど必須処理に失敗したjobの最大claim回数（既定3）、`--stale-after-seconds` は強制終了したprocessing claimを再投入する猶予（既定900秒）である。TCC/Full Disk Accessとlaunchdの導入手順は [Voice Memos デプロイチェックリスト](VOICE_MEMOS_DEPLOY_CHECKLIST.md) を参照。
