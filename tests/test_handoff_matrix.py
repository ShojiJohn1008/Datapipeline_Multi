...
class TestPipelineAndCliMatrix(unittest.TestCase):
    ...
    def test_no_llm_has_sources(self):
        ...
        self.assertIn("📋 URI（長押しでコピー）: `obsidian://open?", reply)
    ...
    def test_icloud_placeholder_scan_timeout_does_not_hang(self):
        with tempfile.TemporaryDirectory() as tmp:
            with mock.patch("knowledge_hub.vault_search.subprocess.run",
                            side_effect=subprocess.TimeoutExpired("find", 0.01)):
                from knowledge_hub.vault_search import icloud_placeholders_exist
                self.assertFalse(icloud_placeholders_exist(Path(tmp), timeout=0.01))
    ...
    def test_keyboard_interrupt_is_reported_without_traceback(self):
        from knowledge_hub.find import main

        stderr = io.StringIO()
        with mock.patch("knowledge_hub.find.config.resolve_vault", return_value=VAULT), \
                mock.patch("knowledge_hub.find._search", side_effect=KeyboardInterrupt), \
                redirect_stderr(stderr):
            self.assertEqual(main(["test", "--no-llm"]), 130)
        self.assertIn("検索を中断しました", stderr.getvalue())
