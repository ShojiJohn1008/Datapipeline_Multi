...
def _format_sources(vault: Path, top: list[FileResult]) -> list[str]:
    ...
    for i, r in enumerate(top, 1):
        ...
        lines.append(f"   📋 URI（長押しでコピー）: `{obsidian_link(vault, r.path)}`")
    return lines
...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        if not args.json:
            print(f"検索中です（最大{int(config.TOTAL_BUDGET)}秒）…", file=sys.stderr, flush=True)
        ...
        print("検索を中断しました。Vaultの内容は変更していません。", file=sys.stderr)
        return 130
